...
const FOREGROUND_INTERVAL_MS = Number(
  process.env.EXPO_PUBLIC_FOREGROUND_LOCATION_INTERVAL_MS ?? 5000,
);
const BACKGROUND_INTERVAL_MS = Number(
  process.env.EXPO_PUBLIC_BACKGROUND_LOCATION_INTERVAL_MS ?? 10000,
);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        // Stale/invalid token - stop hammering the server with it and force
        // a fresh login instead of retrying forever with a dead token.
...
TaskManager.defineTask(LOCATION_TASK_NAME, async ({ data, error }: any) => {
  ...
  if (data) {
    ...
    // Keep local store fresh so a background-triggered SOS (e.g. voice trigger)
    // uses the latest coords instead of stale foreground-only data.
    ...
    // Send location to backend server - same path foreground uses
    try {
      await sendLocationToServer(
        location.coords.latitude,
        location.coords.longitude,
  ...
  try {
    ...
    if (!isRegistered) {
      ...
      await Location.startLocationUpdatesAsync(LOCATION_TASK_NAME, {
        ...
        deferredUpdatesInterval: BACKGROUND_INTERVAL_MS / 2,
